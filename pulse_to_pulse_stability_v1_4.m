...
times = fopen([file_base_name '_times_tmp.txt'],'w');
data = fopen([file_base_name '_data_tmp.txt'],'w');

log_line = 'Time;Ampl';
content = fileread([file_base_name '.txt']);
Nl = length(strfind(content,'#')); % number of data logs
...
% EXPLANATION
% length(strfind(content(ends-8:end-cut),';')) counts every occurrence of
% the character ';'. Since our measurement results use ';' as seperator
% and we only have two values per line, this equals a line count.
% The -1 results from neglecting the header.
% Since we know that every timestamp is measured with the same amount of
% data points dividing Np by Nl gives us the resulting number of points per
% measurement.
Np = length(strfind(content(ends-8:end-cut),';')) - 1;
Np = Np / Nl;
...
%     theo_values = zeros(1,N);
%     log_values  = zeros(1,N);
%     for i=1:N
%         if mod(i,50)==0
%             fprintf('%d\n',i)
%         end
