...
%% Fit Polynomials to the phase and plot the results depending on degree
    ...
    Pv = zeros(max_orders,length(w_Sk));
...
%% Integrate back into the time domain
    ...
    for order=1:max_orders
        ...
        fit_Sk_cplx_opt = sqrt(fit_I_Sk) .* exp(1i*fit_d_opt);

        [t,E]         = Speck_Fourier(l_Sk'*1e-9,Sk_cplx);
        [t_opt,E_opt] = Speck_Fourier(l_Sk'*1e-9,Sk_cplx_opt);
        [fit_t_opt,fit_E_opt] = Speck_Fourier(fit_l_Sk'*1e-9,fit_Sk_cplx_opt);

        t     = t * 1e15;
        t_opt = t_opt * 1e15;
        fit_t_opt = fit_t_opt * 1e15;

        Int     = abs(trapz(t,abs(E).^2));
        Int_opt = abs(trapz(t_opt,abs(E_opt).^2));
        fit_Int_opt = abs(trapz(fit_t_opt,abs(fit_E_opt).^2));

        % Scale the integrals to allow for comparison with Fourier limit
        E     = E .* sqrt(Int_F ./ Int);
        E_opt = E_opt .* sqrt(Int_F ./ Int_opt);
        fit_E_opt = fit_E_opt .* sqrt(fit_Int_F ./ fit_Int_opt);
        fprintf('Order %d: The optimization for the whole range leads to %2.2f percent of the Fourier limit.\n',order,max(abs(E_opt).^2)*100)
        fprintf('Order %d: The optimization for the spectral range of interest leads to %2.2f percent of the Fourier limit.\n',order,max(abs(fit_E_opt).^2)*100)
        Ev(order,:) = E;
        E_optv(order,:) = E_opt;
...
%% Figure part
    if do_plot
        ...
        plot(w_Sk,filtered_p_Sk-P_opt,'c')
        plot([w0 w0],[min(filtered_p_Sk) max(filtered_p_Sk)],'r')
        hold off
        xlim([lower-0.01 higher+0.01])
        ylim([-20 50])
        ...
        % Shift it correctly
        t0 = find_closest_idx(fit_t_opt,0);
        E0 = find_closest_idx(abs(fit_E_opt').^2,max(abs(fit_E_opt').^2));
        plot(fit_t_opt,circshift(abs(fit_E_opt').^2,t0-E0),'r')
        hold off
