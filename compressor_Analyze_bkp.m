...
%     do_output(filename_broad_before,filename_broad_after,filename_broad_sent,filename_broad_Trubetskov2);
    do_output(filename_broad_before,filename_broad_after_Full,filename_broad_sent,filename_broad_Trubetskov2);
end

function do_output(filename_before,filename_after,filename_orig,filename_theo)
    [pulse_info_before,...
     pulse_info_after,...
     common_Speck_before,...
     common_Speck_after,...
     ...
     Int_F] = do_calculate(filename_before,filename_after,filename_orig,filename_theo);
...
function [pulse_info_before,...
          ...
          spectral_phase_theoretical,...
          Int_F] ...
    = do_calculate(filename_before,filename_after,filename_orig,filename_theo)
...
%     Ek_after(:,3) = unwrap(fftshift(Ek_after(:,3)));
    ...
    [Int_comp,t_comp,Ek_comp] = compressor_toTimeDomain(spectral_intensity_original,common_wavelength,spectral_phase_original + spectral_phase_diff);
    ...
    spectral_phase_theoretical = 8 .* spectral_phase_theoretical; % This is necessary because the data was provided as per mirror, 8 bounces in the compressor
    [Int_theo,t_theo,Ek_theo] = compressor_toTimeDomain(spectral_intensity_original,common_wavelength,spectral_phase_original - spectral_phase_theoretical);
    Compression_Info_Theoretical1 = {Int_theo,t_theo,Ek_theo};
    [Int_theo,t_theo,Ek_theo] = compressor_toTimeDomain(spectral_intensity_original,common_wavelength,spectral_phase_before + spectral_phase_theoretical);
    ...
    % graphs correctly
    [Int_F,~,~] = compressor_toTimeDomain(spectral_intensity_original,common_wavelength);
end
...
function do_plot(pulse_info_before,...
    ...
    wavelength_after    = pulse_info_after(:,1);
...
%     phase_expected = tilt_curve(wavelength_before,phase_after,phase_expected);
    figure()
        subplot(2,2,1)
            H2 = zeros(1,4);
            [AX, H1, H2(1)] = plotyy(wavelength_before,intensity_before,wavelength_before,phase_before);
            ...
            xlim(AX(2),[1000 1060])
            ylim(AX(2),[-20 75])
            l = legend(H2,{'Measured Phase', 'Theoretical Phase', 'P_{Measured} + P_{before}', 'P_{Measured} - P_{before}'},'Location','NorthWest');
        subplot(2,2,2)
            H2 = zeros(1,3);
...
%             set(l,'FontSize',F_size_Label);
    ...
    figure()
        ...
        subplot(2,2,2)
            ...
            %l = legend('Measured by T. Pleyer','Location','SouthEast');
            set(l,'FontSize',F_size_Label);
        subplot(2,2,3)
            ...
            %l = legend('Measured by T. Pleyer','Location','NorthWest');
            set(l,'FontSize',F_size_Label);
    ...
    figure()
        subplot(2,2,1)
            [AX, H1, H2] = plotyy(time_orig,tintensity_orig./Int_Orig.*Int_F,time_orig,tphase_orig);
            ...
            xlim(AX(2),[-1e4 1e4])
        subplot(2,2,2)
            [AX, H1, H2] = plotyy(time_theo1,abs(Ek_theo1).^2./Int_theo1.*Int_F,time_theo1,unwrap(-angle(Ek_theo1)));
            ...
            xlim(AX(2),[-1e4 1e4])
        subplot(2,2,3)
            [AX, H1, H2] = plotyy(time_comp,abs(Ek_comp).^2./Int_comp.*Int_F,time_comp,unwrap(-angle(Ek_comp)));
...
function cpulse_info=correct_pulse(pulse_info,varargin)
    ...
    L = length(phase);
    [~,I] = min(phase);
    if (I(1) == 1 || I(1) == L) %Not the most sophisticated check. Only works when the GDD is more dominant then the higher order, which is the case for most of our cases
        sign = 'neg';
    else
        sign = 'pos';
    end
